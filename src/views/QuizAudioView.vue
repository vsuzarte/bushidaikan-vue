<template>
    <q-layout>
      <header-with-logos titulo="Quiz Áudio"></header-with-logos>
      <q-page-container class="q-page-container-audio">
        <div v-if="!finalizado">
          <selecionar-tipos v-if="!quizIniciado" :tipos="tiposStore.tipos" @iniciar-quiz="iniciarQuiz"></selecionar-tipos>
          <questao-audio v-else @continuar-quiz="nextQuestion" :currentQuestion="getCurrentQuestion"></questao-audio>
        </div>
        <div v-else>
  ...
  import QuestaoAudio from '../components/QuestaoAudio.vue';
  import HeaderWithLogos from '@/components/HeaderWithLogos.vue';
  import Placar from '@/components/Placar.vue';
  
  export default {
    components: { SelecionarTipos, QuestaoAudio, HeaderWithLogos, Placar },
    name: 'QuizAudio',
    setup() {
